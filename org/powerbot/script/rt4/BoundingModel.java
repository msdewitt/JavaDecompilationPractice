...
public abstract class BoundingModel
  extends ClientAccessor
{
  ...
  Point centroid(int arg0)
  {
    ...
    int i = x();int j = z();int k = ctx).game.tileHeight(i, j);
    Point localPoint = ctx).game.worldToScreen(i + (arrayOfInt[arg0][0][0] + arrayOfInt[arg0][1][0] + arrayOfInt[arg0][2][0]) / 3, k + (arrayOfInt[arg0][0][1] + arrayOfInt[arg0][1][1] + arrayOfInt[arg0][2][1]) / 3, j + (arrayOfInt[arg0][0][2] + arrayOfInt[arg0][1][2] + arrayOfInt[arg0][2][2]) / 3, 0);
  ...
  public Point centerPoint()
  {
    ...
    int m = 0;
    int n = 0;
    int i1 = x();int i2 = z();int i3 = ctx).game.tileHeight(i1, i2);
    while (n < i) {
    ...
    Point localPoint = ctx).game.worldToScreen(i1 + j / i, i3 + k / i, i2 + m / i, 0);
  ...
  public boolean contains(Point arg0) {
    ...
    int i = x;int j = y;
    int k = x();int m = z();int n = ctx).game.tileHeight(k, m);
    label317:
    for (int[][] arrayOfInt : arrayOfInt1)
    {


      Point[] arrayOfPoint1 = { ctx).game.worldToScreen(k + arrayOfInt[0][0], n + arrayOfInt[0][1], m + arrayOfInt[0][2], 0), ctx).game.worldToScreen(k + arrayOfInt[1][0], n + arrayOfInt[1][1], m + arrayOfInt[1][2], 0), ctx).game.worldToScreen(k + arrayOfInt[2][0], n + arrayOfInt[2][1], m + arrayOfInt[2][2], 0) };
  ...
  public boolean drawWireFrame(Graphics arg0) {
    ...
    for (int[][] arrayOfInt : arrayOfInt1)
    {
      ...
      Point[] arrayOfPoint1 = { ctx).game.worldToScreen(i + arrayOfInt[0][0], k + arrayOfInt[0][1], j + arrayOfInt[0][2], 0), ctx).game.worldToScreen(i + arrayOfInt[1][0], k + arrayOfInt[1][1], j + arrayOfInt[1][2], 0), ctx).game.worldToScreen(i + arrayOfInt[2][0], k + arrayOfInt[2][1], j + arrayOfInt[2][2], 0) };
  ...
  private int firstInViewportIndex(int[][][] arg0, int arg1, int arg2) {
    ...
    if (m < arg2) {
      ...
      Point[] arrayOfPoint2 = arrayOfPoint1;int n = arrayOfPoint2.length; for (int i1 = 0;; i1++) { if (i1 >= n) break label240; Point localPoint = arrayOfPoint2[i1];
        if (!ctx).game.inViewport(localPoint))
          break;
      }
      label240:
