package org.powerbot.bot.rt6.client;

import org.powerbot.bot.Reflector;

public class RenderableNode extends org.powerbot.bot.ReflectProxy
{
  private static final org.powerbot.bot.Reflector.FieldCache jdField_new = new org.powerbot.bot.Reflector.FieldCache();
  private static final org.powerbot.bot.Reflector.FieldCache jdField_void = new org.powerbot.bot.Reflector.FieldCache();
  
  public RenderableNode(Reflector arg0, Object arg1) {
    super(arg0, arg1);
  }
  
  public RenderableNode getNext() {
    return new RenderableNode(reflector, reflector.access(this, new));
  }
  
  public RenderableEntity getEntity() {
    return new RenderableEntity(reflector, reflector.access(this, void));
  }
}
